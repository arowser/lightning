# This dockerfile is meant to cross compile with a x64 machine for a riscv64 host
# It is using multi stage build: 
# * builder: Cross compile c-lightning dependencies, then c-lightning itself with static linking
# * final: Copy the binaries required at runtime
# The resulting image uploaded to dockerhub will only contain what is needed for runtime.
# From the root of the repository, run "docker build -t yourimage:yourtag -f contrib/linuxriscv64.Dockerfile ."
...
RUN apt-get update && apt-get install -y --no-install-recommends ca-certificates autoconf automake build-essential git libtool python python3 wget gnupg dirmngr git \
  libc6-riscv64-cross gcc-riscv64-linux-gnu g++-riscv64-linux-gnu qemu qemu-user-static qemu-user binfmt-support

ENV target_host=riscv64-linux-gnu
...
COPY --from=builder /usr/bin/qemu-risv64-static /usr/bin/qemu-risv64-static
