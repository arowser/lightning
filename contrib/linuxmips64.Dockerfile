# This dockerfile is meant to cross compile with a x64 machine for a mips64 host
# It is using multi stage build: 
# * builder: Cross compile c-lightning dependencies, then c-lightning itself with static linking
# * final: Copy the binaries required at runtime
# The resulting image uploaded to dockerhub will only contain what is needed for runtime.
# From the root of the repository, run "docker build -t yourimage:yourtag -f contrib/linuxmips64.Dockerfile ."
...
RUN apt-get update && apt-get install -y --no-install-recommends ca-certificates autoconf automake build-essential git libtool python python3 wget gnupg dirmngr git \
  libc6-mips64-cross gcc-mips64-linux-gnuabi64 g++-mips64-linux-gnuabi64 qemu qemu-user-static qemu-user binfmt-support

ENV target_host=mips64-linux-gnu
...
RUN curl -L -o qemu-arm-static.tar.gz https://github.com/multiarch/qemu-user-static/releases/download/v3.1.0-2/qemu-mips-static.tar.gz\
    && tar xvf qemu-mips-static.tar.gz \
    && sudo cp qemu-mips-static /usr/bin/qemu-mips-static \
...
COPY --from=builder /usr/bin/qemu-mips64-static /usr/bin/qemu-mips64-static
